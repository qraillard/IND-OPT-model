...
def Capacity_planning(model,t_tt_combinations,s_t_combinations):
    ...
    model.Added_removed_capacity_Ctr=Constraint(model.TECHNOLOGIES_SECTOR,model.AREAS,model.YEAR,rule=Added_removed_capacity_rule)
    ...
    def Tech_type_change_ramp_rule(model,tech,tech_type,sector,area,year):
        if year>min(getattr(model, "YEAR").data()):
            return model.V_technology_tech_type_use_coef_capacity[tech,tech_type,sector,area,year]-\
                    model.V_technology_tech_type_use_coef_capacity[tech,tech_type,sector,area,year-1]<=0.5*model.P_installation_ramp_t[tech,sector,area,year]+\
